...
while {true} do {
	...
	if ((_fobdistance < _distredeploy || count KP_liberation_neararsenal != 0 || count KP_liberation_nearspawn != 0 || (player distance startbase) < 200) && alive player && vehicle player == player) then {
		if (_idact_arsenal == -1) then {
			_idact_arsenal = player addAction ["<t color='#FFFF00'>" + localize "STR_ARSENAL_ACTION" + "</t> <img size='2' image='res\ui_arsenal.paa'/>","[player, player, true] call ace_arsenal_fnc_openBox","",-980,true,true,"","build_confirmed == 0"];
		};
	} else {
		if (_idact_arsenal != -1) then {
